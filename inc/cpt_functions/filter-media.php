...
add_action('wp_ajax_media', 'media_filter_function');
add_action('wp_ajax_nopriv_media', 'media_filter_function');

function media_filter_function()
{
    ...
    $args = array(
        ...
        'post_type' => 'media', // The post type to filter
    ...
    if (isset($_POST['categoryfilter']) && $_POST['categoryfilter'] !== 'all')
        $args['tax_query'] = array(
            array(
                'taxonomy' => 'media-categories', // The taxonomy name
    ...
    if ($query->have_posts()) :
        while ($query->have_posts()) : $query->the_post();
            get_template_part('template-parts', media_cards());
        endwhile;
    else :
        echo '<div class="no-posts-found">There are currently no media to show.</div>';
    endif;
    wp_reset_postdata();
